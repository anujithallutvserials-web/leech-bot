...
# --- Configuration (നിങ്ങളുടെ ടോക്കണും ഐഡികളും ഇവിടെ നൽകുക) ---
...
# ==========================================
# 1. GOFILE LINK HANDLER & EXTRACTOR FIX
# ==========================================
...
async def gofile_link_handler(client: Client, message: Message):
    ...
    await status_msg.edit_text("❌ Failed to fetch or download from Gofile. Please check if the link is active!")

# ==========================================
# 2. MAGNET LINK HANDLER FIX
# ==========================================
@app.on_message(filters.regex(r"magnet:\?xt=urn:btih:") & filters.chat(ALLOWED_GROUP_ID))
async def magnet_link_handler(client: Client, message: Message):
    user = message.from_user
    user_id = user.id
    magnet_link = message.text.split()[0] if len(message.text.split()) > 0 else message.text
    
    status_msg = await message.reply_text("🧲 **Magnet Link detected!** Initializing torrent session...")
    await process_magnet_link(client, status_msg, user_id, user.first_name, magnet_link)
    ...
    try:
        ...
        while not handle.has_metadata():
            ...
            if time.time() - start_time > timeout:
                raise Exception("Timeout reached! Could not fetch metadata. Poor seeders/peers for this magnet.")
...
# ==========================================
# 3. TELEGRAM CHANNEL LEECH COMMAND (`/leech`)
# ==========================================
@app.on_message(filters.command("leech") & filters.chat(ALLOWED_GROUP_ID))
async def leech_telegram_link(client: Client, message: Message):
    args = message.text.split()
    if len(args) < 2:
        await message.reply_text("⚠️ **Please provide a Telegram post/media link!**\n\n**Usage:** `/leech https://t.me/c/xxxx/xxxx` or `/leech https://t.me/channelname/xxxx`")
        return
        
    link = args[1]
    status_msg = await message.reply_text("🔍 **Analyzing Telegram Link...**")
    ...
    try:
        ...
        await status_msg.edit_text(f"❌ **Leech Failed!**\n\n**Reason:** `{str(e)}`")
